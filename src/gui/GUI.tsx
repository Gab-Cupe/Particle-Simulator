...
interface GUIProps {
  ...
  collisibleGround: boolean;
  setCollisibleGround: (val: boolean) => void;
  ...
  const handleSaveConfig = () => {
    const config: SavedConfig = {
      ...
      settings: {
        ...
        collisibleGround: p.collisibleGround,
        showGrid: p.showGrid,
  ...
  return (
    <div className="gui-wrapper">
      <div className="panel-derecho">
          ...
          {openSections.environment && (
            <div className="section-content">
              <div className="button-grid">
                ...
                <button onClick={() => p.setCollisibleGround(!p.collisibleGround)}>
                  Collisible ground?: {p.collisibleGround ? "ON" : "OFF"}
                </button>
              ...
              />
              <label>Time Scale: x{p.timeScale.toFixed(1)}</label>
              <input  
                type="range"
                min="0.3"
                max="4"
                step="0.1"
                value={p.timeScale}
                onChange={(e) => p.setTimeScale(Number(e.target.value))}
              />
            </div>
          )}
        </div>
        ...
        {/* Section: Views */}
        <div className="section">
          <div className="section-header" onClick={() => toggleSection('views')}>
            <h4>Views</h4>
            <span className={`toggle-icon ${openSections.views ? 'open' : ''}`}>▼</span>
          </div>
          {openSections.views && (
            <div className="section-content">
              <div className="button-grid">
                <button onClick={() => p.onViewPreset("iso")}>Isometrica</button>
                <button onClick={() => p.onViewPreset("+x")}>Eje +X</button>
                <button onClick={() => p.onViewPreset("+y")}>Eje +Y</button>
                <button onClick={() => p.onViewPreset("+z")}>Eje +Z</button>
                <button onClick={() => p.onViewPreset("-x")}>Eje -X</button>
                <button onClick={() => p.onViewPreset("-y")}>Eje -Y</button>
                <button onClick={() => p.onViewPreset("-z")}>Eje -Z</button>
              </div>
